...
from moodle_upgrade.inventory import PHP_SETTINGS_SCRIPT, _custom_path_summary, _database_platform, _git_state, collect_inventory
...
def create_fake_moodle(root: Path):
    ...
    (root / "version.php").write_text("<?php\n$version = 2025041400.00;\n$release = '5.0';\n$branch = '500';\n", encoding="utf-8")
...
def test_inventory_reads_php_from_docker_runtime(tmp_path, monkeypatch):
    ...
    create_fake_moodle(root)
    cfg = base_config(root)
    cfg["runtime"] = {"type": "docker", "container": "demo-php-1", "moodle_root": "/var/www/html", "moodledata": "/var/www/moodledata"}
    commands = []
    ...
    def fake_run(command, cwd=None, timeout=10):
        ...
        if command[:3] == ["docker", "exec", "demo-php-1"] and command[3:5] == ["php", "-r"]:
            assert command[5] == PHP_SETTINGS_SCRIPT
            return {"ok": True, "returncode": 0, "stdout": "5000|256M|8", "stderr": ""}
    ...
    assert result["platform"]["php"]["settings"] == {"max_input_vars": "5000", "memory_limit": "256M", "int_size": 8}
...
def test_database_runtime_records_image_version_and_prefix(monkeypatch):
    def fake_run(command, cwd=None, timeout=10):
        if command[:4] == ["docker", "inspect", "-f", "{{.State.Running}}|{{.Config.Image}}"]:
    ...
    monkeypatch.setattr(inventory_module, "_run", fake_run)
    result = _database_platform({"database": {"driver": "mysql", "runtime_container": "demo-db-1", "prefix": "mdl_"}})
    assert result["driver"] == "mysql"
    assert result["prefix"] == "mdl_"
