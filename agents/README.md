...
`manifest.yml` registers eight `AGENT.md` contracts. Their YAML frontmatter defines identity, effect, execution mode, allowed/forbidden capabilities, evidence inputs/outputs and delegation. The Markdown body tells any compatible coding agent how to use that bounded role.
...
documentation-agent   moodle.document, moodle.document.sync
```
...
Run `muk run-agents` to execute successive permitted deterministic actions until the workflow completes or reaches a blocker, external adapter action, or human gate. The runner never supplies its own approval.
