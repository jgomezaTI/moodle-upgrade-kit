...
PHP_SETTINGS_SCRIPT = 'echo ini_get("max_input_vars"),"|",ini_get("memory_limit"),"|",PHP_INT_SIZE;'
...
def _git_state(root: Path) -> dict[str, Any]:
    ...
    return {"is_repo": True, "repo_root": str(repo_root), "branch": branch["stdout"] or None, "head": head["stdout"] or None, "dirty": bool(status["stdout"]) if status["returncode"] is not None else None}
...
def _disk(path: Path) -> dict[str, Any]:
    try:
        ...
        return {"path": str(path), "total_bytes": usage.total, "used_bytes": usage.used, "free_bytes": usage.free, "free_percent": round((usage.free / usage.total) * 100, 2) if usage.total else 0}
...
def _custom_path_summary(root: Path, relative_path: str, repo_root: Path | None, max_files: int) -> dict[str, Any]:
    ...
    if not _path_within(target, boundary):
        ...
        return {"path": relative_path, "exists": False, "resolved_path": str(target), "error": f"custom path escapes allowed {scope}"}
    scope = "moodle" if _path_within(target, root_resolved) else "project"
    base_result = {"path": relative_path, "resolved_path": str(target), "scope": scope}
    if not target.exists():
        return {**base_result, "exists": False, "tracked_by_git": _git_tracks_path(repo_root, target)}
    if target.is_file():
        return {**base_result, "exists": True, "kind": "file", "size_bytes": target.stat().st_size, "tracked_by_git": _git_tracks_path(repo_root, target)}
    file_count = 0
    dir_count = 0
    size_bytes = 0
    ...
    for entry in target.rglob("*"):
        ...
        if file_count >= max_files:
            truncated = True
            break
    return {**base_result, "exists": True, "kind": "directory", "file_count": file_count, "dir_count": dir_count, "size_bytes": size_bytes, "top_extensions": dict(extensions.most_common(10)), "truncated": truncated, "tracked_by_git": _git_tracks_path(repo_root, target)}
...
def _non_core_top_level_candidates(root: Path, excluded_paths: list[Path]) -> list[str]:
    ...
    for child in sorted(root.iterdir()):
        if not child.is_dir() or child.name.startswith("."):
            continue
        if child.name in KNOWN_CORE_TOP_LEVEL or child.name in IGNORED_TOP_LEVEL or child.resolve() in excluded:
            continue
...
def _parse_php_settings(stdout: str) -> dict[str, Any]:
    parts = stdout.split("|")
    if len(parts) != 3:
        return {"max_input_vars": None, "memory_limit": None, "int_size": None}
    max_input_vars, memory_limit, int_size_text = [part.strip() for part in parts]
    try:
        int_size = int(int_size_text)
    except ValueError:
        int_size = None
    return {"max_input_vars": max_input_vars or None, "memory_limit": memory_limit or None, "int_size": int_size}


def _docker_runtime(runtime_cfg: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], list[str], dict[str, Any]]:
    ...
    if running and runtime_root:
        ...
        for name, (flag, path) in {"version.php": ("-f", base / "version.php"), "config.php": ("-f", base / "config.php"), "admin/cli": ("-d", base / "admin" / "cli")}.items():
            markers[name] = _run(["docker", "exec", container, "test", flag, str(path)])["ok"]
    if running:
        php = _run(["docker", "exec", container, "php", "-v"])
        modules_result = _run(["docker", "exec", container, "php", "-m"])
        settings_result = _run(["docker", "exec", container, "php", "-r", PHP_SETTINGS_SCRIPT])
    else:
        unavailable = {"ok": False, "returncode": None, "stdout": "", "stderr": "Docker container is not running"}
        php = unavailable
        modules_result = unavailable
        settings_result = unavailable
    runtime = {"type": "docker", "container": container, "running": running, "image": inspected["image"], "moodle_root": runtime_root, "moodledata": runtime_moodledata, "markers": markers}
    modules = _clean_php_modules(modules_result["stdout"]) if modules_result["ok"] else []
    settings = _parse_php_settings(settings_result["stdout"]) if settings_result["ok"] else _parse_php_settings("")
    return runtime, php, modules, settings


def _runtime_platform(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], list[str], dict[str, Any]]:
    ...
    settings_result = _run(["php", "-r", PHP_SETTINGS_SCRIPT]) if php["ok"] else {"ok": False, "stdout": ""}
    modules = _clean_php_modules(modules_result.get("stdout", "")) if modules_result.get("ok") else []
    settings = _parse_php_settings(settings_result.get("stdout", "")) if settings_result.get("ok") else _parse_php_settings("")
    return {"type": "local"}, php, modules, settings
...
def _database_platform(config: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"driver": driver, "prefix": database_cfg.get("prefix"), "container": container, "running": None, "image": None, "version_line": None}
    if not container:
        return result
...
def collect_inventory(config: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime, php, php_modules, php_settings = _runtime_platform(config)
    ...
    plugin_inventory = _plugins(root, configured_plugin_roots, plugins_cfg.get("custom_paths", [])) if root.exists() else []
    ...
    custom_inventory = [_custom_path_summary(root, path, repo_root, max_files) for path in list(custom_cfg.get("paths", []))]
    ...
    non_core_candidates = _non_core_top_level_candidates(root, [path for path in excluded if path is not None]) if custom_cfg.get("auto_detect_top_level", True) else []
    ...
    cron = {"configured": bool(cron_command), "command": cron_command, "cli_exists": (root / "admin" / "cli" / "cron.php").exists() if root.exists() else False}
    custom_plugin_count = sum(1 for plugin in plugin_inventory if plugin["classification"] == "custom")
    return {
        "identity": {
            "project": config.get("project", {}), "moodle_root": str(root), "moodledata": str(moodledata) if moodledata else None,
            "base_url": moodle_cfg.get("base_url"), "target_version": moodle_cfg.get("target_version"), "markers": marker_state,
            "moodle_version": _read_moodle_version(root) if root.exists() else {},
        },
        "platform": {
            "runtime": runtime,
            "php": {"available": php["ok"], "version_line": php["stdout"].splitlines()[0] if php["stdout"] else None, "modules": php_modules, "settings": php_settings},
            "database": database, "git": git, "disk": disks,
        },
        "plugins": plugin_inventory,
        "custom_code": {"configured_paths": custom_inventory, "non_core_top_level_candidates": non_core_candidates},
        "cron": cron, "findings": [asdict(finding) for finding in findings],
        "summary": {
            "critical": sum(1 for finding in findings if finding.severity == "critical"),
            "warning": sum(1 for finding in findings if finding.severity == "warning"),
            "info": sum(1 for finding in findings if finding.severity == "info"),
            "plugin_count": len(plugin_inventory), "custom_plugin_count": custom_plugin_count,
            "configured_custom_code_count": len(custom_inventory), "non_core_top_level_candidate_count": len(non_core_candidates),
        },
    }
